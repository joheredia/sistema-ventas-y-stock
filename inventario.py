...
class Inventario(tk.Frame):
    ...
    def widgets(self):
        ...
        titulo = tk.Label(self, text="CONTROL DE STOCK", bg="#dddddd", font="lexend 28 bold", anchor="center")
        titulo.pack()
        ...
        labelframe = LabelFrame(frame2, text="Productos", font="lexend 14 bold", bg="lightsteelblue1")
        labelframe.place(x=20, y=30, width=400, height=500)

        lblnombre = Label(labelframe, text="Nombre: ", font="lexend 10 bold", bg="lightsteelblue1")
        lblnombre.place(x=10, y=20)
        self.nombre = ttk.Entry(labelframe, font="lexend 10 bold")
        self.nombre.place(x=140, y=20, width=240, height=40)

        lblproveedor = Label(labelframe, text="Proveedor: ", font="lexend 10 bold", bg="lightsteelblue1")
        lblproveedor.place(x=10, y=80)
        self.proveedor = ttk.Entry(labelframe, font="lexend 10 bold")
        self.proveedor.place(x=140, y=80, width=240, height=40)

        lblprecio = Label(labelframe, text="Precio: ", font="lexend 10 bold", bg="lightsteelblue1")
        lblprecio.place(x=10, y=140)
        self.precio = ttk.Entry(labelframe, font="lexend 10 bold")
        self.precio.place(x=140, y=140, width=240, height=40)

        lblcosto = Label(labelframe, text="Costo: ", font="lexend 10 bold", bg="lightsteelblue1")
        lblcosto.place(x=10, y=200)
        self.costo = ttk.Entry(labelframe, font="lexend 10 bold")
        self.costo.place(x=140, y=200, width=240, height=40)

        lblstock = Label(labelframe, text="Stock: ", font="lexend 10 bold", bg="lightsteelblue1")
        lblstock.place(x=10, y=260)
        self.stock = ttk.Entry(labelframe, font="lexend 10 bold")
        self.stock.place(x=140, y=260, width=240, height=40)

        boton_agregar = tk.Button(labelframe, text="Ingresar", font="lexend 14 bold", bg="#dddddd", command=self.registrar)
        boton_agregar.place(x=80, y=340, width=240, height=40)

        boton_editar = tk.Button(labelframe, text="Editar", font="lexend 14 bold", bg="#dddddd", command=self.editar_producto)
        ...
        scrol_y.pack(side=BOTTOM, fill=X)
        ...
        btn_actualizar = Button(frame2, text="Actualizar Stock", font="sans 14 bold", command=self.actualizar_inventario)
        btn_actualizar.place(x=440, y=480, width=200, height=50)
    ...
    def mostrar(self):
        ...
        for elem in result:
            try:
                precio_arg = "{:,.0f}$".format(float(elem[3])) if elem[3] else ""
                costo_arg = "{:,.0f}$".format(float(elem[4])) if elem[4] else ""
            except ValueError:
                precio_arg = elem[3]
                costo_arg = elem[4]
            self.tre.insert("", 0, text=elem[0], values=(elem[0], elem[1], elem[2], precio_arg, costo_arg, elem[5]))
    ...
    def editar_producto(self):
        ...
        ventana_editar.config(bg="midnight blue")

        lbl_nombre = Label(ventana_editar, text="Nombre: ", font="lexend 14 bold", bg="lightsteelblue1")
        lbl_nombre.grid(row=0, column=0, padx=10, pady=10)
        entry_nombre = Entry(ventana_editar, font="lexend 14 bold")
        entry_nombre.grid(row=0, column=1, padx=10, pady=10)
        entry_nombre.insert(0, item_values[1])

        lbl_proveedor = Label(ventana_editar, text="Proveedor: ", font="lexend 14 bold", bg="lightsteelblue1")
        lbl_proveedor.grid(row=1, column=0, padx=10, pady=10)
        entry_proveedor = Entry(ventana_editar, font="lexend 14 bold")
        entry_proveedor.grid(row=1, column=1, padx=10, pady=10)
        entry_proveedor.insert(0, item_values[2])

        lbl_precio = Label(ventana_editar, text="Precio: ", font="lexend 14 bold", bg="lightsteelblue1")
        lbl_precio.grid(row=2, column=0, padx=10, pady=10)
        entry_precio = Entry(ventana_editar, font="lexend 14 bold")
        entry_precio.grid(row=2, column=1, padx=10, pady=10)
        entry_precio.insert(0, item_values[3].split()[0].replace(",", ""))

        lbl_costo = Label(ventana_editar, text="Costo: ", font="lexend 14 bold", bg="lightsteelblue1")
        lbl_costo.grid(row=3, column=0, padx=10, pady=10)
        entry_costo = Entry(ventana_editar, font="lexend 14 bold")
        entry_costo.grid(row=3, column=1, padx=10, pady=10)
        entry_costo.insert(0, item_values[4].split()[0].replace(",", ""))

        lbl_stock = Label(ventana_editar, text="Stock: ", font="lexend 14 bold", bg="lightsteelblue1")
        lbl_stock.grid(row=4, column=0, padx=10, pady=10)
        entry_stock = Entry(ventana_editar, font="lexend 14 bold")
        ...
        def guardar_cambios():
            ...
            consulta = "UPDATE inventario SET nombre=?, precio=?, proveedor=?, costo=?, stock=? WHERE id=?"
        ...
        btn_guardar = Button(ventana_editar, text="Guardar cambios", font="lexend 14 bold", command=guardar_cambios)
