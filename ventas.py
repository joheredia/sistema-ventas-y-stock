...
class Ventas(tk.Frame):
    ...
    def widgets(self):
        ...
        titulo = tk.Label(self, text="VENTAS", bg="#dddddd", font="lexend 28 bold", anchor="center")
        titulo.pack()
        titulo.place(x=5, y=0, width=1090, height=90)

        frame2 = tk.Frame(self, bg="lightsteelblue1", highlightbackground="gray", highlightthickness=1)
        frame2.place(x=0, y=100, width=1100, height=550)

        lblframe = LabelFrame(frame2, text="Informacion de la venta", bg="lightsteelblue1", fg="black", font="lexend 14 bold")
        lblframe.place(x=10, y=10, width=1060, height=80)

        label_numero_factura = tk.Label(lblframe, text="N Factura: ", bg="lightsteelblue1", fg="black", font="lexend 10 bold")
        label_numero_factura.place(x=10, y=5)
        self.numero_factura = tk.StringVar()

        self.entry_numero_factura = ttk.Entry(lblframe, textvariable=self.numero_factura, state="readonly", font="lexend 10 bold")
        self.entry_numero_factura.place(x=90, y=12, width=100)

        label_nombre = tk.Label(lblframe, text="Productos: ", bg="lightsteelblue1", fg="black", font="lexend 10 bold")
        label_nombre.place(x=200, y=12)
        self.entry_nombre = ttk.Combobox(lblframe, font="lexend 10 bold", state="readonly")
        self.entry_nombre.place(x=280, y=10, width=120)

        self.cargar_productos()

        label_valor = tk.Label(lblframe, text="Precio: ", bg="lightsteelblue1", fg="black", font="lexend 10 bold")
        label_valor.place(x=420, y=12)
        self.entry_valor = ttk.Entry(lblframe, font="lexend 10 bold", state="readonly")
        self.entry_valor.place(x=480, y=10, width=100)

        self.entry_nombre.bind("<<ComboboxSelected>>", self.actualizar_precio)

        label_cantidad = tk.Label(lblframe, text="Cantidad: ", bg="lightsteelblue1", fg="black", font="lexend 10 bold")
        label_cantidad.place(x=600, y=12)
        self.entry_cantidad = ttk.Entry(lblframe, font="lexend 10 bold")
        self.entry_cantidad.place(x=680, y=10, width=100)

        label_unidad = tk.Label(lblframe, text="Unidad: ", bg="lightsteelblue1", fg="black", font="lexend 10 bold")
        label_unidad.place(x=800, y=12)
        self.entry_unidad = ttk.Entry(lblframe, font="lexend 10 bold")
        self.entry_unidad.place(x=880, y=10, width=100)

        treFrame = tk.Frame(frame2, bg="lightsteelblue1")
        ...
        self.tree = ttk.Treeview(treFrame, columns=("Producto", "Precio", "Cantidad", "Unidad", "Subtotal"), show="headings", height=10, yscrollcommand=scrol_y.set, xscrollcommand=scrol_x.set)
        ...
        self.tree.heading("#5", text="Subtotal")

        self.tree.column("Producto", anchor="center")
        self.tree.column("Precio", anchor="center")
        self.tree.column("Cantidad", anchor="center")
        self.tree.column("Unidad", anchor="center")
        self.tree.column("Subtotal", anchor="center")

        self.tree.pack(expand=True, fill=BOTH)

        lblframe1 = LabelFrame(frame2, text="Opciones", bg="#dddddd", font="lexend 10 bold")
        lblframe1.place(x=10, y=380, width=1060, height=100)

        boton_agregar = tk.Button(lblframe1, text="Agregar Articulo", bg="#dddddd", font="lexend 10 bold", command=self.registrar)
        boton_agregar.place(x=50, y=10, width=240, height=50)

        boton_pagar = tk.Button(lblframe1, text="Pagar", bg="#dddddd", font="lexend 10 bold", command=self.abrir_ventana_pago)
        boton_pagar.place(x=400, y=10, width=240, height=50)

        boton_ver_facturas = tk.Button(lblframe1, text="Ver Facturas", bg="#dddddd", font="lexend 10 bold", command=self.abrir_ventana_factura)
        boton_ver_facturas.place(x=750, y=10, width=240, height=50)

        self.label_suma_total = tk.Label(frame2, text="Total a pagar: $0", fg="black", font="lexend 22 bold")
    ...
    def registrar(self):
        ...
        if producto and precio and cantidad and unidad:
            try:
                ...
                self.tree.insert("", "end", values=(producto, f"{precio:.0f}", cantidad, unidad, f"{subtotal:.0f}"))
                ...
                messagebox.showerror("Error", "Cantidad, precio o unidad no válidos")
        else:
    ...
    def abrir_ventana_pago(self):
        ...
        ventana_pago.config(bg="lightsteelblue1")
        ventana_pago.resizable(False, False)

        label_total = tk.Label(ventana_pago, bg="lightsteelblue1", text=f"Total a pagar: $ {self.obtener_total():.0f}", font="lexend 16 bold")
        label_total.place(x=70, y=20)

        label_cantidad_pagada = tk.Label(ventana_pago, bg="lightsteelblue1", text="Cantidad pagada: ", font="lexend 12 bold")
        label_cantidad_pagada.place(x=100, y=90)
        entry_cantidad_pagada = ttk.Entry(ventana_pago, font="lexend 12 bold")
        entry_cantidad_pagada.place(x=100, y=130)

        label_cambio = tk.Label(ventana_pago, bg="lightsteelblue1", text="", font="lexend 12 bold")
        ...
        boton_calcular = tk.Button(ventana_pago, text="Calcular Vuelto", bg="white", font="lexend 12 bold", command=calcular_cambio)
        boton_calcular.place(x=100, y=240, width=240, height=40)

        boton_pagar = tk.Button(ventana_pago, text="Pagar", bg="white", font="lexend 12 bold", command=lambda: self.pagar(ventana_pago, entry_cantidad_pagada, label_cambio))
        ...
        try:
            ...
            try:
                ...
                for child in self.tree.get_children():
                    ...
                    productos.append([producto, precio, cantidad_vendida, unidad, subtotal])

                    c.execute("INSERT INTO ventas (factura, nombre_articulo, valor_articulo, cantidad, unidad, subtotal) VALUES (?, ?, ?, ?, ?, ?)",
                              (self.numero_factura_actual, producto, float(precio), cantidad_vendida, unidad, subtotal))
    ...
    def generar_factura_pdf(self, productos, total, factura_numero, fecha):
        ...
        data = [["Producto", "Precio", "Cantidad", "Unidad", "Subtotal"]] + productos
        table = Table(data)
        ...
        messagebox.showinfo("Factura Generada", f"La factura #{factura_numero} ha sido creado exitosamente")
    ...
    def abrir_ventana_factura(self):
        ...
        ventana_facturas.config(bg="lightsteelblue1")
        ventana_facturas.resizable(False, False)

        facturas = Label(ventana_facturas, bg="lightsteelblue1", text="Facturas Registradas", font="lexend 30 bold")
        facturas.place(x=150, y=15)

        treFrame = tk.Frame(ventana_facturas, bg="lightsteelblue1")
        ...
        tree_facturas = ttk.Treeview(treFrame, columns=("ID", "Factura", "Producto", "Precio", "Cantidad", "Unidad", "Subtotal"), show="headings", height=10, yscrollcommand=scrol_y.set, xscrollcommand=scrol_x.set)
        ...
        tree_facturas.heading("#6", text="Unidad")
        tree_facturas.heading("#7", text="Subtotal")

        tree_facturas.column("ID", width=100, anchor="center")
        tree_facturas.column("Factura", width=100, anchor="center")
        tree_facturas.column("Producto", width=100, anchor="center")
        tree_facturas.column("Precio", width=100, anchor="center")
        tree_facturas.column("Cantidad", width=100, anchor="center")
        tree_facturas.column("Unidad", width=100, anchor="center")
        tree_facturas.column("Subtotal", width=100, anchor="center")
